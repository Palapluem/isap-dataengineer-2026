...
- exact normalized name จับคู่ได้ 158 ชื่อ จากชื่อ OCSC 258 ชื่อ และ CGD 568 ชื่อ ขอบเขตของสองแหล่งไม่เท่ากัน จึงไม่ควรตีความตัวเลขนี้เป็น accuracy
...
- ผล extraction ปัจจุบันได้ 5,801 staging rows
...
- ผล extraction ปัจจุบันได้ 2,913 staging rows
...
- `pytest` ปัจจุบันมี 13 tests ครอบคลุม date conversion, header normalization, source transforms, idempotency, manifest comparison, ZIP extraction และ sync orchestration
...
| CGD staging/mart rows | 2,913 |
| OCSC staging/mart rows | 5,801 |
| Core DQ issues | 0 |
